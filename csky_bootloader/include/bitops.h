...
extern unsigned int comp_find_first_bit(const unsigned long *block, unsigned int bits, int swap);
extern unsigned int comp_find_last_bit(const unsigned long *block, unsigned int bits, int swap);
extern unsigned int comp_find_first_zero(const unsigned long *block, unsigned int bits, int swap);
extern unsigned int comp_find_last_zero(const unsigned long *block, unsigned int bits, int swap);
extern unsigned int comp_find_next_bit(const unsigned long *addr1, const unsigned long *addr2, unsigned int bits, unsigned int start, unsigned long invert, int swap);
extern unsigned int comp_find_prev_bit(const unsigned long *addr1, const unsigned long *addr2, unsigned int bits, unsigned int start, unsigned long invert, int swap);
...
static inline unsigned int find_first_bit(const unsigned long *addr, unsigned int bits)
{
    if (small_const_nbits(bits))
        ...
        return comp_find_first_bit(addr, bits, 0);
}
#endif
...
static inline unsigned int find_last_bit(const unsigned long *addr, unsigned int bits)
{
    if (small_const_nbits(bits))
        ...
        return comp_find_last_bit(addr, bits, 0);
}
#endif
...
static inline unsigned int find_first_zero(const unsigned long *addr, unsigned int bits)
{
    if (small_const_nbits(bits))
        ...
        return comp_find_first_zero(addr, bits, 0);
}
#endif
...
static inline unsigned int find_last_zero(const unsigned long *addr, unsigned int bits)
{
    if (small_const_nbits(bits))
        ...
        return comp_find_last_zero(addr, bits, 0);
}
#endif
...
{                                                                                           \
    ...
    if (small_const_nbits(bits))                                                            \
        ...
        return comp_find_first_bit(addr, bits, 1);                                       \
}                                                                                           \
                                                                                            \
static inline unsigned int                                                                  \
find_last_bit_##name(const void *addr, unsigned int bits)                                   \
{                                                                                           \
    ...
    if (small_const_nbits(bits))                                                            \
        ...
        return comp_find_last_bit(addr, bits, 1);                                        \
}                                                                                           \
                                                                                            \
static inline unsigned int                                                                  \
find_first_zero_##name(const void *addr, unsigned int bits)                                 \
{                                                                                           \
    ...
    if (small_const_nbits(bits))                                                            \
        ...
        return comp_find_first_zero(addr, bits, 1);                                      \
}                                                                                           \
                                                                                            \
static inline unsigned int                                                                  \
find_last_zero_##name(const void *addr, unsigned int bits)                                  \
{                                                                                           \
    ...
    if (small_const_nbits(bits))                                                            \
        ...
        return comp_find_last_zero(addr, bits, 1);                                       \
}                                                                                           \
                                                                                            \
static inline unsigned int                                                                  \
find_next_bit_##name(const void *addr, unsigned int bits, unsigned int offset)              \
{                                                                                           \
    ...
    if (small_const_nbits(bits)) {                                                          \
        ...
        return val ? ffsuf(val) : bits;                                                     \
                                                                                            ...
                                                                                            \
    return comp_find_next_bit(addr, NULL, bits, offset, 0UL, 1);                         \
}                                                                                           \
                                                                                            \
static inline unsigned int                                                                  \
find_prev_bit_##name(const void *addr, unsigned int bits, unsigned int offset)              \
{                                                                                           \
    ...
    if (small_const_nbits(bits)) {                                                          \
        ...
        return val ? flsuf(val) : bits;                                                     \
                                                                                            ...
                                                                                            \
    return comp_find_prev_bit(addr, NULL, bits, offset, 0UL, 1);                         \
}                                                                                           \
                                                                                            \
static inline unsigned int                                                                  \
find_next_zero_##name(const void *addr, unsigned int bits, unsigned int offset)             \
